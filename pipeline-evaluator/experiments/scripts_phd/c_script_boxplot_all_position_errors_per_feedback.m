...
cached = 0;
if exist(strcat(data_path, map_name, '_amcl_position_error_3.mat')) == 2
  cached = cached + 1;
endif

if exist(strcat(data_path, map_name, '_amcl_position_error_4.mat')) == 2
  cached = cached + 1;
endif
...
if exist(strcat(data_path, map_name, '_amcl_position_error_6.mat')) == 2
  cached = cached + 1;
endif
...
if cached == 0
  ...
  for m=3:6
    ...
    amcl_position_errors_c{m} = nonzeros(amcl_position_errors .* (amcl_sizes == 1))';
    dft_position_errors_c{m} = nonzeros(dft_position_errors .* (dft_sizes == 1))';
...
% Positions
errors = {amcl_position_errors_c{5}, dft_position_errors_c{5}, ...
          amcl_position_errors_c{6}, dft_position_errors_c{6}, ...
          amcl_position_errors_c{4}, dft_position_errors_c{4}, ...
          amcl_position_errors_c{3}, dft_position_errors_c{3}};

errors_sizes = { 1*ones(1,size(amcl_position_errors_c{5},2)),  2*ones(1,size(dft_position_errors_c{5},2)), ...
                 5*ones(1,size(amcl_position_errors_c{6},2)),  6*ones(1,size(dft_position_errors_c{6},2)), ...
                 9*ones(1,size(amcl_position_errors_c{4},2)), 10*ones(1,size(dft_position_errors_c{4},2)), ...
                13*ones(1,size(amcl_position_errors_c{3},2)), 14*ones(1,size(dft_position_errors_c{3},2))};
...
if strcmp(map_name, 'corridor') == 1
  set (gca, 'ytick', [0:0.05:0.15], 'yticklabel', {'$0.0$','$0.05$','$0.10$','$0.15$'});
...
if strcmp(map_name, 'warehouse') == 1
  set (gca,  'ytick', [0:10:40], 'yticklabel', {'$0.0$','$10.0$','$20.0$','$30.0$','$40.0$'});
  line([0 15], [0.00 0.00], 'linestyle', ':', 'color', 'm', 'linewidth', linewidth)
...
title('Κατανομή σφαλμάτων εκτίμησης θέσης ανά μέθοδο ανάδρασης [m]');
...
if strcmp(map_name, 'corridor') == 1
  set (gca, 'ytick', [0.0, 0.01, 0.02], 'yticklabel', {'$0.0$','$0.01$','$0.02$'});
  line([0 15], [0.00 0.00], 'linestyle', '-', 'color', 'm', 'linewidth', linewidth)
  line([0 0],  [0.00 0.02], 'linestyle', '-', 'color', 'm', 'linewidth', linewidth)
  line([0 15], [0.02 0.02], 'linestyle', '-', 'color', 'm', 'linewidth', linewidth)
  line([15 15],[0.00 0.02], 'linestyle', '-', 'color', 'm', 'linewidth', linewidth)
...
title(strcat(upper(map_name), ' position errors per feedback method'));
xlabel('Μέθοδος ανάδρασης');
ylabel('$\overline{\|e\|_{2,i}}$, $i = \{1,2,\dots,N\}$');

img_file= strcat(save_eps_path, map_name, '_all_position_errors_per_feedback.eps');
